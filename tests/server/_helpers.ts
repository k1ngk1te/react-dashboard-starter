...
import { router, env } from '../../server/base';
...
export async function getCsrfCredentials(app: ReturnType<typeof createApp>) {
  ...
  const csrfCookie = setCookieHeaders?.find((c) => c.startsWith(env.CSRF_TOKEN));
  const csrfValue = csrfCookie?.split(';')[0].split('=')[1] ?? '';
  return { csrfValue, cookieHeader: `${env.CSRF_TOKEN}=${csrfValue}` };
}
